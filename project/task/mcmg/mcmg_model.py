# Modified version of: https://github.com/DobromirM/MCMG/blob/main/model.py
...
# MCMG model
class Model(Module):
    def __init__(self, hidden_size, lr, l2, step, n_head, k_blocks, args, POI_n_node, cate_n_node, regi_n_node,
                 time_n_node, POI_dist_n_node, regi_dist_n_node, len_max):
    ...
    def get_channel_scores(self, POI_hidden, POI_mask, cate_hidden, cate_mask, regi_hidden, regi_mask,
                           group_label_inputs):
        ...
        for k in range(self.k_blocks):
            ...
            POI_attn_output, POI_attn_output_weights = self.multihead_attn(POI_attn_output, POI_attn_output,
                                                                           POI_attn_output)
            POI_attn_output = POI_attn_output.transpose(0, 1)
            POI_attn_output = self.ffn(POI_attn_output)
            cate_attn_output = cate_attn_output.transpose(0, 1)
            cate_attn_output, cate_attn_output_weights = self.multihead_attn(cate_attn_output, cate_attn_output,
                                                                             cate_attn_output)
            cate_attn_output = cate_attn_output.transpose(0, 1)
            cate_attn_output = self.ffn(cate_attn_output)
            regi_attn_output = regi_attn_output.transpose(0, 1)
            regi_attn_output, regi_attn_output_weights = self.multihead_attn(regi_attn_output, regi_attn_output,
                                                                             regi_attn_output)
            regi_attn_output = regi_attn_output.transpose(0, 1)
        ...
        # region-aware weighting strategy
        for i in range(len(POI_hidden)):
            if group_label_inputs[i] == 1:
                POI_represent[i] = w_p_g1 * POI_represent[i].clone() + w_c_g1 * cate_represent[i].clone() + w_r_g1 * \
                                   regi_represent[i].clone()
            elif group_label_inputs[i] == 2:
                POI_represent[i] = w_p_g2 * POI_represent[i].clone() + w_c_g2 * cate_represent[i].clone() + w_r_g2 * \
                                   regi_represent[i].clone()
            else:
...
def forward(model, i, POI_adj_matrix, POI_data, cate_data, regi_data, time_data, POI_dist_data, regi_dist_data,
            group_label):
    ...
    POI_hidden, cate_hidden, regi_hidden, time_hidden, POI_dist_hidden, regi_dist_hidden = model(POI_all_items,
                                                                                                 POI_adj_matrix,
                                                                                                 cate_all_items,
                                                                                                 regi_all_items,
                                                                                                 time_all_items,
                                                                                                 POI_dist_all_items,
                                                                                                 regi_dist_all_items)
    # incorporate time and distance factors
    POI_seq_hidden = torch.stack([torch.matmul(POI_hidden[i], model.w_p) for i in POI_inputs]) + torch.stack(
        [torch.matmul(time_hidden[i], model.w_p_t) for i in time_inputs]) + torch.stack(
        [torch.matmul(POI_dist_hidden[i], model.w_p_d) for i in POI_dist_inputs])
    cate_seq_hidden = torch.stack([torch.matmul(cate_hidden[i], model.w_c) for i in cate_inputs]) + torch.stack(
        [torch.matmul(time_hidden[i], model.w_c_t) for i in time_inputs])
    regi_seq_hidden = torch.stack([torch.matmul(regi_hidden[i], model.w_r) for i in regi_inputs]) + torch.stack(
        [torch.matmul(time_hidden[i], model.w_r_t) for i in time_inputs]) + torch.stack(
        [torch.matmul(regi_dist_hidden[i], model.w_r_d) for i in regi_dist_inputs])
    ...
    POI_score_result, cate_score_result, regi_score_result = model.get_channel_scores(POI_seq_hidden, POI_mask,
                                                                                      cate_seq_hidden, cate_mask,
                                                                                      regi_seq_hidden, regi_mask,
                                                                                      group_label_inputs)
...
def model_train(model, train_loader):
    ...
    POI_adj_matrix = train_loader.POI_adj_matrix
    POI_train_data = train_loader.POI_train_data
    cate_train_data = train_loader.cate_train_data
    regi_train_data = train_loader.regi_train_data
    time_train_data = train_loader.time_train_data
    POI_dist_train_data = train_loader.POI_dist_train_data
    regi_dist_train_data = train_loader.regi_dist_train_data
    group_label_train_valid = train_loader.group_label_train
    ...
    for i, j in zip(slices, np.arange(len(slices))):
        ...
        POI_groundtruth, POI_scores, cate_groundtruth, cate_scores, regi_groundtruth, regi_scores, group_label_inputs = forward(
            model, i, POI_adj_matrix, POI_train_data, cate_train_data, regi_train_data, time_train_data,
            POI_dist_train_data, regi_dist_train_data, group_label_train_valid)
...
def model_test(model, test_loader):
    ...
    POI_adj_matrix = test_loader.POI_adj_matrix
    POI_test_data = test_loader.POI_test_data
    cate_test_data = test_loader.cate_test_data
    regi_test_data = test_loader.regi_test_data
    time_test_data = test_loader.time_test_data
    POI_dist_test_data = test_loader.POI_dist_test_data
    regi_dist_test_data = test_loader.regi_dist_test_data
    group_label_test = test_loader.group_label_test

    slices = POI_test_data.generate_batch(model.batch_size)
    POI_groundtruth, POI_scores, cate_groundtruth, cate_scores, regi_groundtruth, regi_scores, group_label_inputs = forward(
        model, slices[0], POI_adj_matrix, POI_test_data, cate_test_data, regi_test_data, time_test_data,
        POI_dist_test_data, regi_dist_test_data, group_label_test)
    ...
    for i in slices:
        POI_groundtruth, POI_scores, cate_groundtruth, cate_scores, regi_groundtruth, regi_scores, group_label_inputs = forward(
            model, i, POI_adj_matrix, POI_test_data, cate_test_data, regi_test_data, time_test_data, POI_dist_test_data,
        ...
        for i in range(len(cate_sub_items_1)):
            for j in range(len(cate_sub_items_1[i])):
                if ((cate_groundtruth[i] - 1) == cate_sub_items_1[i][j]) and (group_label_inputs[i] == 1):
                    ...
                    break
                elif ((cate_groundtruth[i] - 1) == cate_sub_items_1[i][j]) and (group_label_inputs[i] == 2):
        ...
        for i in range(len(regi_sub_items_1)):
            for j in range(len(regi_sub_items_1[i])):
                if ((regi_groundtruth[i] - 1) == regi_sub_items_1[i][j]) and (group_label_inputs[i] == 1):
                    ...
                    break
                elif ((regi_groundtruth[i] - 1) == regi_sub_items_1[i][j]) and (group_label_inputs[i] == 2):
        ...
        for i in range(len(cate_sub_items_5)):
            for j in range(len(cate_sub_items_5[i])):
                if ((cate_groundtruth[i] - 1) == cate_sub_items_5[i][j]) and (group_label_inputs[i] == 1):
                    ...
                    break
                elif ((cate_groundtruth[i] - 1) == cate_sub_items_5[i][j]) and (group_label_inputs[i] == 2):
        ...
        for i in range(len(regi_sub_items_5)):
            for j in range(len(regi_sub_items_5[i])):
                if ((regi_groundtruth[i] - 1) == regi_sub_items_5[i][j]) and (group_label_inputs[i] == 1):
                    ...
                    break
                elif ((regi_groundtruth[i] - 1) == regi_sub_items_5[i][j]) and (group_label_inputs[i] == 2):
        ...
        for i in range(len(cate_sub_items_10)):
            for j in range(len(cate_sub_items_10[i])):
                if ((cate_groundtruth[i] - 1) == cate_sub_items_10[i][j]) and (group_label_inputs[i] == 1):
                    ...
                    break
                elif ((cate_groundtruth[i] - 1) == cate_sub_items_10[i][j]) and (group_label_inputs[i] == 2):
        ...
        for i in range(len(regi_sub_items_9)):
            for j in range(len(regi_sub_items_9[i])):
                if ((regi_groundtruth[i] - 1) == regi_sub_items_9[i][j]) and (group_label_inputs[i] == 1):
                    ...
                    break
                elif ((regi_groundtruth[i] - 1) == regi_sub_items_9[i][j]) and (group_label_inputs[i] == 2):
